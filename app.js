...
const NEUTRAL_CATS = DEBT_CATS.concat(["transfer_out","transfer_in","adjust_up","adjust_down"]);
function isDebtTx(t){ return NEUTRAL_CATS.indexOf(t.category)>=0; }
const EXTRA_CATS = [
  ...
  { id: "transfer_out", label: "تحويل صادر", icon: "🔄" },
  { id: "transfer_in", label: "تحويل وارد", icon: "🔄" },
  { id: "adjust_down", label: "تسوية (نقص)", icon: "⚖️" },
  { id: "adjust_up", label: "تسوية (زيادة)", icon: "⚖️" },
];
...
function walletsHTML() {
  ...
  let html = `<section class="aw-ledger aw-ledger-wallet">
    ...
    ${ws.length>=2?`<button class="aw-btn primary aw-transfer-btn" data-act="transfer">🔄 تحويل بين المحافظ</button>`:""}
          ...
          return `<div class="aw-person-row ${st?"settled":""}">
            ...
            ${(d.pays&&d.pays.length)?`<div class="aw-pays">
              ...
              ${d.pays.slice().sort((x,y)=>x.date<y.date?1:-1).map((pp)=>`<div class="aw-pay-row">
                <span class="aw-pay-amt">${fmt(pp.amount)} ${esc(d.cur)}${pp.payCur?` <span class="aw-pay-fx">(${fmt(pp.payAmount)} ${esc(pp.payCur)})</span>`:""}</span>
...
function openTransferModal(){
  const cur = state.activeCur;
  const ws = state.wallets.filter((w)=>w.cur===cur);
  if(ws.length<2){ alert("بدك محفظتين على الأقل بنفس العملة."); return; }
  let fromId = ws[0].id, toId = ws[1].id;
  const body = `
    <div class="aw-mini-hint" style="margin-bottom:10px">نقل فلوس بين محافظك. ما بينحسب صادر ولا وارد — بس بينقل الرصيد.</div>
    <label class="aw-field-label">من محفظة</label>
    <div class="aw-wallet-pick" id="trFrom"></div>
    <label class="aw-field-label">إلى محفظة</label>
    <div class="aw-wallet-pick" id="trTo"></div>
    <label class="aw-field-label">المبلغ</label>
    <div class="aw-amount-wrap"><span class="aw-amount-cur">${esc(cur)}</span>
      <input class="aw-amount-input" id="trAmount" type="number" inputmode="decimal" placeholder="0" autofocus></div>
    <div class="aw-net-line" id="trHint"></div>
    <label class="aw-field-label">التاريخ</label>
    <input class="aw-input" id="trDate" type="date" value="${todayStr()}">
    <label class="aw-field-label">ملاحظة (اختياري)</label>
    <input class="aw-input" id="trNote" type="text" placeholder="مثلاً: سحب من البنك">
    <div class="aw-sheet-actions"><button class="aw-btn ghost" id="trCancel">إلغاء</button><button class="aw-btn primary" id="trSave">تحويل</button></div>`;
  const m = modalShell("🔄 تحويل بين المحافظ", body);
  const s = m.sheet;
  function drawPicks(){
    s.querySelector("#trFrom").innerHTML = ws.map((w)=>`<button class="aw-wpick-btn ${fromId===w.id?"on":""}" data-from="${w.id}"><span>${walletIcon(w.name)}</span>${esc(w.name)}</button>`).join("");
    s.querySelector("#trTo").innerHTML = ws.map((w)=>`<button class="aw-wpick-btn ${toId===w.id?"on":""} ${w.id===fromId?"dim":""}" data-to="${w.id}"><span>${walletIcon(w.name)}</span>${esc(w.name)}</button>`).join("");
    s.querySelectorAll("[data-from]").forEach((b)=>b.onclick=()=>{ fromId=b.dataset.from; if(toId===fromId){ const alt=ws.find((w)=>w.id!==fromId); if(alt) toId=alt.id; } drawPicks(); hint(); });
    s.querySelectorAll("[data-to]").forEach((b)=>b.onclick=()=>{ if(b.dataset.to===fromId) return; toId=b.dataset.to; drawPicks(); hint(); });
  }
  function hint(){
    const a=parseFloat(s.querySelector("#trAmount").value)||0;
    const fw=state.wallets.find((w)=>w.id===fromId), tw=state.wallets.find((w)=>w.id===toId);
    const el=s.querySelector("#trHint");
    if(a>0 && fw && tw){
      const fb=walletCurrent(fw), tb=walletCurrent(tw);
      el.innerHTML=`${esc(fw.name)}: ${fmt(fb)} ← <b>${fmt(fb-a)}</b> &nbsp;•&nbsp; ${esc(tw.name)}: ${fmt(tb)} ← <b>${fmt(tb+a)}</b>`;
      el.style.display="block";
    } else el.style.display="none";
  }
  drawPicks();
  s.querySelector("#trAmount").oninput=hint;
  s.querySelector("#trCancel").onclick=m.close;
  s.querySelector("#trSave").onclick=()=>{
    const amt=parseFloat(s.querySelector("#trAmount").value);
    if(!(amt>0)) return;
    if(fromId===toId) return;
    const fw=state.wallets.find((w)=>w.id===fromId), tw=state.wallets.find((w)=>w.id===toId);
    const dt=s.querySelector("#trDate").value, nt=(s.querySelector("#trNote").value||"").trim();
    const tag=nt?" — "+nt:"";
    const tm=nowTime(), gid=uid();
    state.transactions.push({ id:uid(), type:"expense", amount:amt, cur, category:"transfer_out", account:"personal",
      walletId:fromId, date:dt, time:tm, transfer:gid, note:`تحويل إلى ${tw.name}${tag}` });
    state.transactions.push({ id:uid(), type:"income", amount:amt, cur, category:"transfer_in", account:"personal",
      walletId:toId, date:dt, time:tm, transfer:gid, note:`تحويل من ${fw.name}${tag}` });
    save(); m.close(); render();
  };
}

function openReconcileModal(wallet){
  const curBal = walletCurrent(wallet);
  const body = `
    <div class="aw-mini-hint" style="margin-bottom:10px">قارن الرصيد الحقيقي اللي بإيدك مع اللي بالتطبيق. لو في فرق، التطبيق بيسجّل حركة تسوية بالفرق.</div>
    <div class="aw-person-sum in"><span>الرصيد بالتطبيق</span><b>${fmt(curBal)} ${esc(wallet.cur)}</b></div>
    <label class="aw-field-label">الرصيد الحقيقي (اللي عدّيته فعلاً)</label>
    <div class="aw-amount-wrap"><span class="aw-amount-cur">${esc(wallet.cur)}</span>
      <input class="aw-amount-input" id="rcActual" type="number" inputmode="decimal" placeholder="${curBal}" autofocus></div>
    <div class="aw-net-line" id="rcDiff"></div>
    <label class="aw-field-label">ملاحظة (اختياري)</label>
    <input class="aw-input" id="rcNote" type="text" placeholder="مثلاً: جرد آخر الشهر">
    <div class="aw-sheet-actions"><button class="aw-btn ghost" id="rcCancel">إلغاء</button><button class="aw-btn primary" id="rcSave">تسجيل التسوية</button></div>`;
  const m = modalShell("⚖️ مطابقة — "+esc(wallet.name), body);
  const s = m.sheet;
  const inp=s.querySelector("#rcActual"), diffEl=s.querySelector("#rcDiff");
  function upd(){
    const v=parseFloat(inp.value);
    if(isNaN(v)){ diffEl.style.display="none"; return; }
    const d=Math.round((v-curBal)*100)/100;
    if(d===0){ diffEl.innerHTML="مطابق تماماً ✓ ما في فرق."; }
    else if(d>0){ diffEl.innerHTML=`في <b>زيادة ${fmt(d)}</b> ${esc(wallet.cur)} — رح تنسجل حركة تسوية بالزيادة.`; }
    else { diffEl.innerHTML=`في <b>نقص ${fmt(Math.abs(d))}</b> ${esc(wallet.cur)} — رح تنسجل حركة تسوية بالنقص.`; }
    diffEl.style.display="block";
  }
  inp.oninput=upd;
  s.querySelector("#rcCancel").onclick=m.close;
  s.querySelector("#rcSave").onclick=()=>{
    const v=parseFloat(inp.value); if(isNaN(v)) return;
    const d=Math.round((v-curBal)*100)/100;
    if(d!==0){
      const nt=(s.querySelector("#rcNote").value||"").trim();
      state.transactions.push({ id:uid(), type: d>0?"income":"expense", amount:Math.abs(d), cur:wallet.cur,
        category: d>0?"adjust_up":"adjust_down", account:"personal", walletId:wallet.id,
        date:todayStr(), time:nowTime(),
        note:`تسوية رصيد ${wallet.name}${nt?" — "+nt:""}` });
    }
    save(); m.close(); render();
  };
}

function openMatchModal(wallet){
  const m = modalShell("🏦 مطابقة كشف البنك — "+esc(wallet.name), `<div id="mtBody"></div>`);
  const body = m.sheet.querySelector("#mtBody");
  function draw(){
    const items = state.transactions
      .filter((t)=>t.walletId===wallet.id && inViewPeriod(t))
      .sort((a,b)=>(a.date<b.date?1:a.date>b.date?-1:(a.id<b.id?1:a.id>b.id?-1:0)));
    const matched = items.filter((t)=>t.matched);
    const unmatched = items.filter((t)=>!t.matched);
    const sum=(arr)=>arr.reduce((s,t)=>s+(t.type==="income"?t.amount:-t.amount),0);
    const opening = Number(wallet.opening)||0;
    const matchedBal = opening + sum(matched);
    const bookBal = walletCurrent(wallet);

    const row=(t)=>{ const info=catInfo(t.category);
      return `<label class="aw-mt-row ${t.matched?"on":""}">
        <input type="checkbox" class="aw-mt-cb" data-mt="${t.id}" ${t.matched?"checked":""}>
        <span class="aw-mt-main">
          <span class="aw-mt-top">
            <span class="aw-mt-amt ${t.type==="income"?"in":"out"}">${t.type==="income"?"+":"−"}${fmt(t.amount)}</span>
            <span class="aw-mt-cat">${info.icon} ${esc(info.label)}</span>
          </span>
          <span class="aw-mt-sub">${esc(t.date)}${t.time?" "+esc(fmtTime(t.time)):""}${t.note?" • "+esc(t.note):""}</span>
        </span>
      </label>`; };

    body.innerHTML = `
      <div class="aw-mini-hint" style="margin-bottom:10px">أشّر ✓ على كل حركة لقيتها بكشف البنك. الباقي بيضل «مش مطابق» — وهاد اللي لازم تراجعه.</div>
      <div class="aw-mt-grid">
        <div class="aw-mt-box"><span>رصيد المطابَق</span><b class="in">${fmt(matchedBal)}</b></div>
        <div class="aw-mt-box"><span>رصيد التطبيق</span><b>${fmt(bookBal)}</b></div>
        <div class="aw-mt-box"><span>الفرق</span><b class="${Math.abs(bookBal-matchedBal)<0.005?"in":"out"}">${fmt(bookBal-matchedBal)}</b></div>
      </div>
      <div class="aw-mini-hint" style="text-align:center;margin:8px 0 14px">
        مطابَق: ${matched.length} • مش مطابق: ${unmatched.length} — الفترة: ${esc(periodLabel())}
      </div>
      <div class="aw-list-head"><span>مش مطابقة</span><span class="aw-list-count">${unmatched.length}</span></div>
      <div class="aw-mt-list">${unmatched.length?unmatched.map(row).join(""):`<div class="aw-empty">كل الحركات مطابَقة ✓</div>`}</div>
      ${matched.length?`<div class="aw-list-head" style="margin-top:14px"><span>مطابَقة ✓</span><span class="aw-list-count">${matched.length}</span></div>
        <div class="aw-mt-list">${matched.map(row).join("")}</div>`:""}
      <div class="aw-sheet-actions">
        <button class="aw-btn ghost" id="mtClose">إغلاق</button>
        <button class="aw-btn" id="mtClear">مسح كل التأشير</button>
      </div>`;

    body.querySelectorAll("[data-mt]").forEach((cb)=>cb.onchange=()=>{
      const t=state.transactions.find((x)=>x.id===cb.dataset.mt);
      if(t){ if(cb.checked) t.matched=true; else delete t.matched; save(); draw(); }
    });
    body.querySelector("#mtClose").onclick=m.close;
    body.querySelector("#mtClear").onclick=()=>confirmDialog("مسح التأشير عن كل حركات هالفترة؟", ()=>{
      items.forEach((t)=>{ delete t.matched; }); save(); draw();
    });
  }
  draw();
}
...
function openWalletDetail(walletId){
  ...
  function draw(){
    ...
    body.innerHTML = `
      ...
      <button class="aw-btn" id="wdMatch" style="width:100%;margin-top:8px">🏦 مطابقة كشف البنك</button>
      <div class="aw-sheet-actions"><button class="aw-btn ghost" id="wdClose">إغلاق</button><button class="aw-btn" id="wdRec">⚖️ مطابقة الرصيد</button><button class="aw-btn primary" id="wdEdit">✏️ تعديل المحفظة</button></div>`;
    body.querySelector("#wdClose").onclick=m.close;
    body.querySelector("#wdMatch").onclick=()=>{ m.close(); openMatchModal(wl); };
    body.querySelector("#wdRec").onclick=()=>{ m.close(); openReconcileModal(wl); };
...
function openPayModal(debt, onDone) {
  ...
  let walletId = "";
  let payCur = debt.cur;          // العملة اللي استلمت/دفعت فيها
  let rate = 1;                   // كم يساوي 1 من payCur بعملة الدين

  const body = `
    <div class="aw-pay-rem">المتبقّي: <b>${fmt(rem)} ${esc(debt.cur)}</b></div>

    <label class="aw-field-label">${mine?"استلمت بأي عملة؟":"دفعت بأي عملة؟"}</label>
    ${curPickerHTML(payCur,"pay")}

    <label class="aw-field-label">المبلغ <span id="pCurLbl">(${esc(debt.cur)})</span></label>
    <div class="aw-amount-wrap"><span class="aw-amount-cur" id="pCurSym">${esc(debt.cur)}</span>
      <input class="aw-amount-input" id="pAmount" type="number" inputmode="decimal" placeholder="0" value="${rem}" autofocus></div>

    <div id="pFxWrap" style="display:none">
      <label class="aw-field-label">سعر الصرف — كم <b id="pFxFrom">${esc(debt.cur)}</b> بالـ<b id="pFxTo">${esc(debt.cur)}</b>؟</label>
      <div class="aw-amount-wrap"><span class="aw-amount-cur">×</span>
        <input class="aw-amount-input" id="pRate" type="number" inputmode="decimal" placeholder="3.7"></div>
      <div class="aw-net-line" id="pFxLine"></div>
    </div>

    <label class="aw-field-label">${mine?"لأي محفظة دخلت الفلوس؟":"من أي محفظة دفعت؟"} (اختياري)</label>
    <div class="aw-wallet-pick" id="pWallets"></div>
  ...
  const s = m.sheet;
  const amtI=s.querySelector("#pAmount"), rateI=s.querySelector("#pRate");

  function drawWallets(){
    const ws = state.wallets.filter((w)=>w.cur===payCur);
    s.querySelector("#pWallets").innerHTML =
      ws.map((w)=>`<button class="aw-wpick-btn ${walletId===w.id?"on":""}" data-w="${w.id}"><span>${walletIcon(w.name)}</span>${esc(w.name)}</button>`).join("")
      + `<button class="aw-wpick-btn ${walletId===""?"on":""}" data-w="">بدون محفظة</button>`
      + (ws.length?"":`<div class="aw-mini-hint">ما في محافظ بعملة ${esc(payCur)}.</div>`);
    s.querySelectorAll("#pWallets [data-w]").forEach((b)=>b.onclick=()=>{ walletId=b.dataset.w; drawWallets(); });
  }
  function fxUpdate(){
    const diff = payCur!==debt.cur;
    s.querySelector("#pFxWrap").style.display = diff?"block":"none";
    s.querySelector("#pCurSym").textContent = payCur;
    s.querySelector("#pCurLbl").textContent = "("+payCur+")";
    s.querySelector("#pFxFrom").textContent = payCur;
    s.querySelector("#pFxTo").textContent = debt.cur;
    const line=s.querySelector("#pFxLine");
    if(diff){
      const a=parseFloat(amtI.value)||0, r=parseFloat(rateI.value)||0;
      if(a>0&&r>0){
        const conv=Math.round(a*r*100)/100;
        const after=Math.max(0, Math.round((rem-conv)*100)/100);
        line.innerHTML = `${fmt(a)} ${esc(payCur)} × ${r} = <b>${fmt(conv)} ${esc(debt.cur)}</b> — الباقي بعدها: <b>${fmt(after)} ${esc(debt.cur)}</b>`;
        line.style.display="block";
      } else { line.style.display="none"; }
    } else line.style.display="none";
  }
  s.querySelector('[data-curpick="pay"]').querySelectorAll("[data-cv]").forEach((b)=>b.onclick=()=>{
    payCur=b.dataset.cv;
    s.querySelectorAll('[data-curpick="pay"] [data-cv]').forEach((x)=>x.classList.toggle("on", x.dataset.cv===payCur));
    walletId=""; if(payCur===debt.cur){ amtI.value=rem; } else { amtI.value=""; }
    drawWallets(); fxUpdate();
  });
  amtI.oninput=fxUpdate; rateI.oninput=fxUpdate;
  drawWallets(); fxUpdate();

  s.querySelector("#pCancel").onclick=m.close;
  s.querySelector("#pSave").onclick=()=>{
    const raw=parseFloat(amtI.value); if(!(raw>0))return;
    const diff = payCur!==debt.cur;
    const r = diff ? parseFloat(rateI.value) : 1;
    if(diff && !(r>0)){ alert("اكتب سعر الصرف."); return; }
    // المبلغ بعملة الدين (اللي بينخصم من الدين)
    let amt = Math.round(raw*r*100)/100;
    amt = Math.min(rem, amt);
    const pnote=(s.querySelector("#pNote").value||"").trim();
    const fxNote = diff ? `${fmt(raw)} ${payCur} × ${r}` : "";
    const fullNote = [pnote, fxNote].filter(Boolean).join(" — ");

    const entry={ id:uid(), amount:amt, date:todayStr(), time:nowTime(), note:fullNote, walletId:walletId||null,
                  payCur: diff?payCur:undefined, payAmount: diff?raw:undefined, rate: diff?r:undefined };
    state.debts = state.debts.map((d)=> d.id===debt.id
      ? {...d, paid: Math.min(d.amount,(d.paid||0)+amt), pays: (d.pays||[]).concat([entry]) }
      : d);
    if (walletId) {
      // الحركة بتنسجل بعملة المحفظة (عملة الدفع) وبالمبلغ الفعلي
      state.transactions.push({ id:uid(), type: mine?"income":"expense", amount:raw, cur:payCur,
        category: mine?"debt_collect":"debt_payment", account:"personal", walletId,
        date:todayStr(), time:nowTime(),
        note:(mine?"تحصيل من ":"تسديد لـ ")+debt.name+(fullNote?" — "+fullNote:"") });
    }
